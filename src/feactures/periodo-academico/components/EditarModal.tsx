...
// Esquema de validación (igual que en AgregarModal)
...
interface EditarPeriodoAcademicoModalProps {
  isOpen: boolean;
  periodo: any;
  onClose: () => void;
  onUpdate?: () => void; // Callback opcional para pasar el periodo actualizado al padre
}

export const EditarPeriodoAcademicoModal: React.FC<EditarPeriodoAcademicoModalProps> = ({ 
  isOpen, 
  periodo,
  onClose,
  onUpdate
}) => {
  const { updatePeriodo } = usePeriodoAcademico();
  const { 
    register, 
    handleSubmit, 
    formState: { errors }, 
  ...
  useEffect(() => {
    if (periodo) {
      ...
      setValue("fechaInicio", periodo.fechaInicio);
      setValue("fechaFin", periodo.fechaFin);
    }
  }, [periodo, setValue]);

  const onSubmit = async (data: z.infer<typeof periodoSchema>) => {
    try {
      const result = await updatePeriodo(periodo.id, {
        nombre: data.nombre,
        fechaInicio: data.fechaInicio,
        fechaFin: data.fechaFin
      });
      if (onUpdate) {
        onUpdate(); // Llamar al callback para actualizar el periodo en el padre
      }
      ...
      <div className="w-full max-w-2xl bg-base-100 rounded-lg shadow-xl overflow-hidden animate-fadeIn">
        ...
        <div className="px-6 pt-5 pb-4 border-b border-base-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-base-content">Editar Periodo Académico</h3>
              <p className="text-sm mt-1 text-base-content/70">
                Modifique la información del periodo
              </p>
            </div>
          </div>
        </div>
